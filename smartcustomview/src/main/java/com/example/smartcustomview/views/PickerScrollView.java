...
import com.example.smartcustomview.bean.Pickers;
...
public class PickerScrollView extends View {
	...
	private int mColorText = 0xfdfdfd;
...
//		mMinTextSize = mMaxTextSize / 2.0f;
	...
	private void init() {
		...
		mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);//文字样式
		mPaint.setStyle(Style.FILL);
		mPaint.setTextAlign(Align.CENTER);//滑动时文字显示的方向
...
//			mDataList.get(indexs);
//			String textData = 
			canvas.drawText(mDataList.get(indexs).getShowConetnt(), baseline, x, mPaint);
		}
	...
	private void drawOtherText(Canvas canvas, int position, int type) {
		float d = (float) (MARGIN_ALPHA * mMinTextSize * position + type
				* mMoveLen);
		float scale = parabola(mViewHeight / 4.0f, d);//数字显示的透明度
		float size = (mMaxTextSize - mMinTextSize) * scale + mMinTextSize;
		mPaint.setTextSize(size);
		mPaint.setAlpha((int) ((mMaxTextAlpha - mMinTextAlpha) * scale + mMinTextAlpha));
		float y = (float) (mViewHeight / 2.0 + type * d);//中间文字的突出程度
		FontMetricsInt fmi = mPaint.getFontMetricsInt();
		float baseline = (float) (y - (fmi.bottom / 2.0 + fmi.top / 2.0));

		int indexs = mCurrentSelected + type * position;
		String textData = mDataList.get(indexs).getShowConetnt();
		canvas.drawText(textData,  baseline,(float) (mViewWidth / 2.0), mPaint);//整个画布的高度宽度  大小s
	}
